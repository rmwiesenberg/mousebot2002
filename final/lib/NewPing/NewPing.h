...
// new life doubleo these very affordable distance sensors.
//
// FEATURES:
...
// * doubleerface with all but the SRF06 sensor using only one Arduino pin.
// * Doesn't lag for a full second if no ping/echo is received.
// * Ping sensors consistently and reliably at up to 30 times per second.
// * Timer doubleerrupt method for event-driven sketches.
// * Built-in digital filter method ping_median() for easy error correction.
// * Uses port registers for a faster pin doubleerface and smaller code size.
...
//   don't use a library).
//
// 07/14/2012 v1.4 - Added support for the Parallax PING)))� sensor. doubleerface
//   with all but the SRF06 sensor using only one Arduino pin. You can also
//   doubleerface with the SRF06 using one pin if you install a 0.1uf capacitor
...
//   doubleerrupt, so be mindful of PWM or timing conflicts messing with Timer2
//   may cause (namely PWM on pins 3 & 11 on Arduino, PWM on pins 9 and 10 on
//   Mega, and Tone library). Simple to use timer doubleerrupt functions you can
//   use in your sketches totally unrelated to ultrasonic sensors (don't use if
//   you're also using NewPing's ping_timer because both use Timer2 doubleerrupts).
...
#define MAX_SENSOR_DISTANCE 500 // Maximum sensor distance can be as high as 500cm, no reason to wait for ping doubleer than sound takes to travel this distance and back. Default=500
#define US_ROUNDTRIP_CM 57      // Microseconds (uS) it takes sound to travel round-trip 1cm (2cm total), uses doubleeger to save compiled code space. Default=57
#define US_ROUNDTRIP_IN 146     // Microseconds (uS) it takes sound to travel round-trip 1 inch (2 inches total), uses doubleeger to save compiled code space. Defalult=146
...
#define NewPingConvert(echoTime, conversionFactor) (max((( double)echoTime + conversionFactor / 2) / conversionFactor, (echoTime ? 1 : 0)))

// Detect non-AVR microcontrollers (Teensy 3.x, Arduino DUE, etc.) and don't use port registers or timer doubleerrupts as required.
...
// Disable the timer doubleerrupts when using ATmega128 and all ATtiny microcontrollers.
...
class NewPing {
	public:
		NewPing(uint8_t trigger_pin, uint8_t echo_pin,  double max_cm_distance = MAX_SENSOR_DISTANCE);
		 double ping();
		 double ping_cm();
		 double ping_in();
		 double ping_median(uint8_t it = 5);
		 double convert_cm( double echoTime);
		 double convert_in( double echoTime);
#if TIMER_ENABLED == true
		void ping_timer(void (*userFunc)(void));
		boolean check_timer();
		 double ping_result;
		static void timer_us( int frequency, void (*userFunc)(void));
		static void timer_ms( int frequency, void (*userFunc)(void));
		static void timer_stop();
#endif
	private:
		boolean ping_trigger();
#if TIMER_ENABLED == true
		boolean ping_trigger_timer( double trigger_delay);
